...
/*****
Modifications by Butchu
- make some pie plots offcenter
- Added sqrt, cbrt, and qdrt option for y-plot axis
- Changed ticking formula for sqrt Y axis for a better looking plot
- Got TTF fonts to work properly (they did not load)
- Corrected calculations of legend widths when using TTF fonts
- Changed 'imagecreate' with 'imagecreatetruecolor', when possible (imagecreate can not deal with TTF fonts on some Linux platforms)
- Set error_reporting to 0 instead of ALL
- Replace all occurences of 'split' by 'explode'
- 09/2009 do not plot if range is smaller that one degree (causes section to cover the whole pie)
*****/
...
// Butchu, 04/2008, changed error_reporting(E_ALL); to error_reporting(0);
error_reporting(0);
...
class PHPlot {
	...
	*/
	...
	var $xscale_type = 'linear';        // linear, log, sqrt, cbtr, qdrt
	var $yscale_type = 'linear';

//Fonts
	var $use_ttf  = TRUE;                  // Use True Type Fonts?
	var $ttf_path = '/Img/Font';                    // Default path to look in for TT Fonts. 
	var $default_ttfont = 'DejaVuSans.ttf';
...
//Colors and styles       (all colors can be array (R,G,B) or named color)
	...
	var $data_colors = array('#E500FF', '#4300FF', '#00C5FF', '#36D958', '#FFF400', '#FFAA00', '#FF0400', '#0072BC');
	var $error_bar_colors = array('#00AEEF', '#00A99D', '#00A651', '#8DC63F', '#FFF200', '#F7941D', '#ED1C24', '#0072BC');
//	Butchu: changed the Rainbow after suggestion from Martin Hoffmann to another, more visible rainbow
//	var $data_colors = array('#00AEEF', '#00A99D', '#00A651', '#8DC63F', '#FFF200', '#F7941D', '#ED1C24', '#0072BC');
//	Butchu: changed default colors to Rainbow after suggestion from Martin Hoffmann
...
//	var $error_bar_colors = array('SkyBlue', 'green', 'orange', 'blue', 'orange', 'red', 'violet', 'azure1');
	...
	// ADDED BY Butchu IN ORDER TO BE ABLE TO MOVE PIE PLOTS
	var $pie_x1_pos = 0;
	var $pie_y1_pos = 0;
	var $pie_x2_pos = 0;
	var $pie_y2_pos = 0;
...
//////////////////////////////////////////////////////
	...
	*/
	function PHPlot($which_width=600, $which_height=400, $which_output_file=NULL, $which_input_file=NULL) 
	{
		...
		// Butchu 02/2008: use ImageCreateTrueColor if possible
...
/////////////////////////////////////////////
	...
	*/
	function SetTTFPath($which_path)
	{
		...
		// Butchu 01/2008
		// which_path was simply the path to the file, included what
		// is in the top definitions
		$which_path = $which_path.$this->ttf_path;
	...
	*/
	function SetDefaultFonts() 
	{
		...
		if ($this->use_ttf) {
				...
				// Butchu 01 2008, increased font size on labels from 6 to 8
	...
	*/
	function SetFont($which_elem, $which_font, $which_size = 12) 
	{
		// TTF:
		if ($this->use_ttf) {
				// Butchu 01/2008, force default_ttfont on $which_font to avoid garbage
...
/////////////////////////////////////////////
	...
	*/
	function CheckOption($which_opt, $which_acc, $which_func)
	{
		...
		if ($asked == '')
				return '';
		// Butchu, 10/2009, replaced eregi by stripos since eregi is going away
		// if (@ eregi($asked, $which_acc)) {
		if (@ strpos(strtolower($which_acc),strtolower($asked)) !== FALSE) {
				return $asked;
		} else {
	...
	*/
	...
	/*!
	* Added by Butchu
	* Pie plot are always centered, I do not know why, to I created a small 
	* function to move them
	* Specifies the absolute (relative to image's up/left corner) position
	* of of the plot
	*/
	function SetPiePlotPixels($x1, $y1, $x2, $y2) 
	{ 
		$this->pie_x1_pos = $x1;
		$this->pie_y1_pos = $y1;
	$this->pie_x2_pos = $x2;
	...
	*/
	...
	{
		$this->xscale_type = $this->CheckOption($which_xst, 'linear, log, sqrt, cbrt, qdrt', __FUNCTION__);
		return TRUE;
	}

	function SetYScaleType($which_yst) 
	{ 
		$this->yscale_type = $this->CheckOption($which_yst, 'linear, log, sqrt, cbrt, qdrt', __FUNCTION__);
		return TRUE;
	}
...
//////////////////////////////////////////////////////////
	...
	*/
	function CalcMargins() 
	{
		...
		if ($this->use_ttf) {
				...
				//echo "Max is $ylab and size is ".$this->y_tick_label_width."<br>";
		}
		// Fixed fonts:
		else { 
				// Maximum X axis label height
	...
	*/
	function SetPlotAreaWorld($xmin=NULL, $ymin=NULL, $xmax=NULL, $ymax=NULL) 
	{
		...
		if (($this->yscale_type == 'sqrt')||($this->yscale_type == 'cbrt')||($this->yscale_type == 'qdrt')) {
				//extra error checking
				if ($ymin < 0) {
					$ymin = 1;
				}
				if ($ymax < 0) {
					$this->PrintError('SetPlotAreaWorld(): Square, cubic, and quadratic root plots need data greater or equal to 0');
					return FALSE;
				}
		}
	...
	*/
	function CalcTranslation() 
	{
		...
		} else if ($this->xscale_type == 'sqrt') {
				$this->xscale = ($this->plot_area_width)/(sqrt($this->plot_max_x) - sqrt($this->plot_min_x));
		} else if ($this->xscale_type == 'cbrt') {
				$this->xscale = ($this->plot_area_width)/(pow($this->plot_max_x, 1./3) - pow($this->plot_min_x,1./3));
		} else if ($this->xscale_type == 'qdrt') {
				$this->xscale = ($this->plot_area_width)/(pow($this->plot_max_x, 1./4) - pow($this->plot_min_x,1./4));
		} else {
		...
		} else if ($this->yscale_type == 'sqrt') {
				$this->yscale = ($this->plot_area_height)/(sqrt($this->plot_max_y) - sqrt($this->plot_min_y));
		} else if ($this->yscale_type == 'cbrt') {
				$this->yscale = ($this->plot_area_height)/(pow($this->plot_max_y,1./3) - pow($this->plot_min_y,1./3));
		} else if ($this->yscale_type == 'qdrt') {
				$this->yscale = ($this->plot_area_height)/(pow($this->plot_max_y,1./4) - pow($this->plot_min_y,1./4));
		}else {
		...
		if ($this->xscale_type == 'log') {
				...
				$this->plot_origin_x = $this->plot_area[0] - ($this->xscale * sqrt($this->plot_min_x) );
		} else if ($this->xscale_type == 'cbrt') {
				$this->plot_origin_x = $this->plot_area[0] - ($this->xscale * pow($this->plot_min_x,1./3) );
		} else if ($this->xscale_type == 'qdrt') {
				$this->plot_origin_x = $this->plot_area[0] - ($this->xscale * pow($this->plot_min_x,1./4) );
		} else {
		...
		if ($this->yscale_type == 'log') {
				...
				$this->plot_origin_y = $this->plot_area[3] + ($this->yscale * sqrt($this->plot_min_y));
		} else if ($this->yscale_type == 'cbrt') {
				$this->plot_origin_y = $this->plot_area[3] + ($this->yscale * pow($this->plot_min_y,1./3));
		} else if ($this->yscale_type == 'qdrt') {
				$this->plot_origin_y = $this->plot_area[3] + ($this->yscale * pow($this->plot_min_y,1./4));
		} else {
	...
	*/
	function xtr($x_world) 
	{
		...
		if ($this->xscale_type == 'log') {
				...
				$x_pixels = $this->plot_origin_x + sqrt($x_world) * $this->xscale ;
		} else if ($this->xscale_type == 'cbrt') {
				$x_pixels = $this->plot_origin_x + pow($x_world,1./3) * $this->xscale ;
		}else if ($this->xscale_type == 'qdrt') {
				$x_pixels = $this->plot_origin_x + pow($x_world,1./4) * $this->xscale ;
		} else {
	...
	*/
	function ytr($y_world) 
	{
		if ($this->yscale_type == 'log') {
				...
				//minus because GD defines y = 0 at top. doh!
				$y_pixels =  $this->plot_origin_y - sqrt($y_world) * $this->yscale ;
		} else if ($this->yscale_type == 'cbrt') {
				//minus because GD defines y = 0 at top. doh!
				$y_pixels =  $this->plot_origin_y - pow($y_world,1./3) * $this->yscale ;
		} else if ($this->yscale_type == 'qdrt') {
				//minus because GD defines y = 0 at top. doh!
				$y_pixels =  $this->plot_origin_y - pow($y_world,1./4) * $this->yscale ;
		} else {
	...
	*/
	function DrawYTicks() 
	{
		...
		if ($this->y_tick_increment) {
				...
				// Changed by Butchu 2005/06 to accomodate sqrt scale
				if ($this->yscale_type == "sqrt") {
					// $y_tmp = $y_start + pow($num_ticks_done*$delta_y,2)
					$delta_y = (sqrt($this->plot_max_y,2) - sqrt($this->plot_min_y,2)) / $this->num_y_ticks-2;
				} elseif ($this->yscale_type == "cbrt") {
					$delta_y = (pow($this->plot_max_y,1./3) - pow($this->plot_min_y,1./3)) / $this->num_y_ticks-2;
				} elseif ($this->yscale_type == "qdrt") {
					$delta_y = (pow($this->plot_max_y,1./4) - pow($this->plot_min_y,1./4)) / $this->num_y_ticks-2;
				} else {
				...
				// Changed by Butchu 2005/06 to accomodate sqrt scale
				if ($this->yscale_type == "sqrt") {
					$delta_y = (sqrt($this->plot_max_y) - sqrt($this->plot_min_y)) / 10;
				} elseif ($this->yscale_type == "cbrt") {
					$delta_y = (pow($this->plot_max_y,1./3) - pow($this->plot_min_y,1./3)) / 10;
				} elseif ($this->yscale_type == "qdrt") {
					$delta_y = (pow($this->plot_max_y,1./4) - pow($this->plot_min_y,1./4)) / 10;
				} else {
		...
		$y_start = (double)$this->plot_min_y;
		// Changed by Butchu 2005/06 to accomodate sqrt scale
		if ($this->yscale_type == "sqrt") {
			// If SQRT scale, ticks will go as y = sqrt (y^2 + delta)
		...
		// Butchu 2005/06 Trying to play around y_ticks... so we can have better scaling for sqrt axis...
		//for (;$y_tmp < $y_end; $y_tmp += $delta_y) {
		$num_ticks_done=0;
		while ($y_tmp < $y_end) {
				//echo "We will have a tick at $y_tmp\n<br>";
				...
				if ($this->yscale_type == "sqrt") {
					// If we have a sqrt scale, we apply a different kind of scaling!
					$num_ticks_done += 1;
					$y_tmp = $y_start + pow($num_ticks_done*$delta_y,2);
				} elseif ($this->yscale_type == "cbrt") {
					// If we have a sqrt scale, we apply a different kind of scaling!
					$num_ticks_done += 1;
					$y_tmp = $y_start + pow($num_ticks_done*$delta_y,3);
				} elseif ($this->yscale_type == "qdrt") {
					// If we have a sqrt scale, we apply a different kind of scaling!
					$num_ticks_done += 1;
					$y_tmp = $y_start + pow($num_ticks_done*$delta_y,4);
				} else {
					$y_tmp += $delta_y;
				}
		}
	...
	*/
	function DrawLegend($which_x1, $which_y1, $which_boxtype) 
	{
		...
		if ($this->use_ttf) {
				...
				// $char_w = $size[0];
				// Butchu 02/2008: more accurate check for legend width
				// Look for the largest legend text.
				$size = 0;
				foreach ($this->legend as $leg) {
					$thissize = $this->TTFBBoxSize($this->legend_font['size'], 0,
													$this->legend_font['font'], $leg);
					if ($thissize[0] > $size) $size = $thissize[0];
				}
				$char_wtotal = $size;
				$size = $this->TTFBBoxSize($this->legend_font['size'], 0,
													$this->legend_font['font'], '_');
				$char_w = $size[0]; // Char width is used to plot the legend boxes
				$size = $this->TTFBBoxSize($this->legend_font['size'], 0,
				...
				$width = $char_wtotal + 40; // Room for box and symbols
		}
		// Fixed fonts:
		else {
...
/////////////////////////////////////////////
	...
	*/
	...
	{
		// CHANGED BY Butchu
		if ($this->hard_pos_pie == 1) {
	$xpos = $this->pie_x1_pos + ($this->pie_x2_pos- $this->pie_x1_pos)/2;
	$ypos = $this->pie_y1_pos + ($this->pie_y2_pos- $this->pie_y1_pos)/2;
	$diameter = min($this->pie_x2_pos- $this->pie_x1_pos, $this->pie_y2_pos- $this->pie_y1_pos);
	$radius = $diameter/2;
		} else {
		$xpos = $this->plot_area[0] + $this->plot_area_width/2;
		...
		for ($h = $this->shading; $h >= 0; $h--) {
				...
				foreach ($sumarr as $val) {
					...
					// Butchu, 09/2009 do not plot if range is smaller that one degree (causes section to cover the whole pie)
					// Draw the slice       
					if ($val>1) ImageFilledArc($this->img, $xpos, $ypos+$h, $diameter, $diam2, 
...
function array_pad_array(&$arr, $size, $arr2=NULL)
{
	...
	while (count($arr) < $size)
		...
		// Butchu: correction for php 5
		$arr = array_merge((array)$arr, (array)$arr2);        // append
}
?>
